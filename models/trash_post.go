...
func (r *TrashPostRepository) Create(post *TrashPost) error {
	query := `
        INSERT INTO trash_posts (user_id, location, description, trail)
        VALUES (?, ?, ?, ?)
        RETURNING id, created_at`
	return r.db.QueryRow(query, post.UserID, post.Location, post.Description, post.Trail).Scan(&post.ID, &post.CreatedAt)
}
...
func (r *TrashPostRepository) GetByDateRange(start, end time.Time) ([]*TrashPost, error) {
	query := `
        SELECT tp.id, tp.user_id, tp.location, tp.description, COALESCE(tp.trail, ''), tp.created_at,
	...
	for rows.Next() {
		p := &TrashPost{}
		u := &User{}
		if err := rows.Scan(&p.ID, &p.UserID, &p.Location, &p.Description, &p.Trail, &p.CreatedAt, &u.ID, &u.Name, &u.Email, &u.IsAdmin, &u.CreatedAt, &u.UpdatedAt); err != nil {
			return nil, err
		}
		p.User = u
